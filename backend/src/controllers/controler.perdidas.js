...
export const update_perdida = async (req, res) => {
  ...
  try {
    ...
    if (!id || isNaN(Number(id))) {
      return res.status(400).json({
        message: "El ID de la pérdida es requerido y debe ser un número válido."
      });
    }
    ...
    // Validar motivo contra ENUM permitido en la tabla
    const motivosValidos = ["mortalidad", "enfermedad", "accidente", "otro"];
    if (!motivosValidos.includes(motivo)) {
      return res.status(400).json({
        message: `El motivo debe ser uno de los siguientes: ${motivosValidos.join(", ")}`
      });
    }

    // Validar fecha (que sea válida y no futura opcionalmente)
    if (isNaN(new Date(fecha_perdida).getTime())) {
      return res.status(400).json({
        message: "La fecha de pérdida no es válida."
      });
    }
